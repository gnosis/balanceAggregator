...
describe("safeBridgeModule", async () => {
    ...
    const baseSetup = deployments.createFixture(async () => {
        ...
        const Uniswap = await hre.ethers.getContractFactory("Mock");
        const mock = await Mock.deploy();
        const uniswap = await hre.ethers.getContractAt("IAdapter", mock.address);
        await mock.givenMethodReturnUint(amb.interface.getSighash("getBalance"), 20);
